...
from .fileio import read_gyre
from scipy import integrate
import pandas as pd
import string
# load some tools from SYDOSU to do asteroseismic things
from .scaling_relation import logg_MR, loggteff2numax, numax, dnu_MR, freq_dyn, numax_sun, dnu_sun, L_sun, R_sun, M_sun
from .dnu_util import dnu_e
from .dnu_correlation import get_model_full
from .smooth_c import smooth_c
from .util import freq_cut

def make_data(attrs, arrs):
    '''
    reformats attrs and arrs from read_gyre() into a version that plot_modes.py can work with.
    also adds 'count' to arrs, which is a sequential numbering of each mode.
    Inputs:
    attrs : the first element of the tuple output from read_gyre() 
    arrs : the second " " " " " from read_gyre()
    Outputs:
    attrs : pd DataFrame
    arrs: pd DataFrame
    '''
    # depending on which version of GYRE is being used, try or the except clause will be used.
    try:
...
def return_modes(profile, ad, visibility_thresh=50.0):
    '''
    Returns rows from <profile>'s arrs DataFrame corresponding only to visible modes.

    Inputs
    profile : str
     the profile name of the file. 
    ad : str
     Either 'nad' or 'ad' for non-adiabatic or adiabatic GYRE results
    [ visibility_thresh : float ]
     Default 50.0. A mode must have a visibility greater than this to be deemed visible.
    Outputs
    arrs_vis : pd DataFrame
     Just the visible modes from <arrs> (created by return_mode).
    Examples
     If the file is called profile50.data.GYRE.gyre_nad.eigval.h5 then <profile> = profile50.data.GYRE and <ad> = 'nad'.
    '''
    # by default, all modes are assumed to be invisible

    eigval_file = profile + '.gyre_'+ad+'.eigval.h5'
    print('reading in')
    print(eigval_file)
    attrs, arrs = read_gyre(eigval_file)
    attrs, arrs = make_data(attrs, arrs)    
    arrs['ratio'] = np.nan
    ...
    for count in arrs['count']:
        ...
        arrs['ratio'].iloc[count] = return_mode(ind_file)
        

    arrs_vis = arrs.loc[arrs['ratio'] > visibility_thresh]
    return arrs_vis
    
def return_mode(ind_file):
    '''
    
    Return the visibility of the mode (a float).
    Inputs
    ind_file : str
     The name of the individual mode file. (E.g., 'profile50.data.GYRE.gyre_nad.mode-00028.h5').
    Outputs
     ratio : float
    The visibility of the mode. A higher <ratio> means it is more visible at the surface than in the core.
    Examples
    ratio = return_mode('profile50.data.GYRE.gyre_nad.mode-00028.h5')
    Would give the visiblity for the 28th mode in the arrs DataFrame (from make_data) for the profile50.data.GYRE.gyre_nad.eigval.h5 star.
    '''
    
    if os.path.exists(ind_file):
        meta_mode, mode = read_gyre(ind_file)
    else:
        print("WARNING: no individual mode file found: {}.".format(ind_file))

        print("assuming no mode is visible by returning visibility of nan.".format(ind_file))
        return np.nan
        return None
...
def main(profile = 'profile3.data', model=0, debug=False, ad='ad', plot=False, surface_corr=False, method='weighted'):
    '''
    ...
    # but if GYRE has been run on that profile...
        ...
        dnu_scal = dnu_MR(attrs['M_star'].__array__(), attrs['R_star'].__array__())
        count = 1
        ratio = []
        eta = []
        inertia = []
    ...
    with open('{}.{}.astero.in'.format(profile,ad), 'w+') as f:
...
if __name__ == '__main__':
    ...
    main(debug=args.debug, profile=args.profile, plot=args.plot)
