...
def dnu_theory(ells, freqs, numax=None, use_all=False, method='weighted', surface_corr=False, E=None, debug=False):
    '''
...
def small_dnu_theory(ells, freqs, numax=None, use_all=False, method='weighted', surface_corr=False, E=None, debug=False):
    ...
    '''
    # compute an initial guess for the large frequency separation using l == 0 modes
    small_dnu = np.mean(freqs[ells == 0] - freqs[ells == 2])
    ...
    if numax is not None:
        ...
        ns = ns[np.where(np.abs(_freqs.apply(lambda x: x.real)  - numax) < dnu.real*n_dnu)[0]]
        _freqs = _freqs.loc[np.abs(_freqs.apply(lambda x: x.real)  - numax) < dnu.real*n_dnu]
        _ells = _ells.loc[np.abs(_ells.apply(lambda x: x.real)  - numax) < dnu.real*n_dnu]
        ...
        try:
            ...
            if weighted:
                # JCZ 130919
                # these are the meid-points between the modes
                centers = (_freqs.loc[_ells == 0] + _freqs.loc[_ells == 2])/2.0
                # width of the Gaussian. from BAM paper Table 1
                # JCZ 130919
                # !!! multiplying by a factor of 3 to basically relax the wieghting. if wnat the full weight, just put to 1.
                # !!! figure out effect of surface effect........
                ...
                small_dnu = np.sum( (_freqs[_ells == 0] - _freqs[_ells == 2])*weights)/np.sum(weights)
                # OK actually changing this to do least-squares fitting like white+ 2011, as dennis recommended.
                # JCZ 240919
                # they still weight by exponential, so that still works, but they weight the actual frequencies, not the center between radial modes, and they use 0.25*numax as the width
                # JCZ 040221
...
def main(profile = 'profile3.data', model=0, debug=False, ad='ad', plot=False, surface_corr=False, method='weighted'):
    '''
    Will save a file called '<profile>.astero.in', which has dnu and numax information.
    ...
    # but if GYRE has been run on that profile...
        ...
        if plot:
            ...
            for l,freq,damping,n_g,E_p,E_g in zip(arrs['l'], arrs['freq'].apply(lambda x: x.real), arrs['freq'].apply(lambda x: x.imag), arrs['n_g'], arrs['E_p'], arrs['E_g']):
                if True:
                    # JCZ 300817
                    ...
                    meta_mode, mode = read_gyre(profile + '.gyre_'+ad+'.mode-{:05d}.h5'.format(count))
            ...
            ell = 3
            n = int(width*2*2)
            plt.plot(f_grid, get_model_full(f_grid, dnu, numax, ell, n, rot=0.0, epsilon=-dnu*1.58, i=20.,envelope_width=np.inf)*amp, color='black')
            plt.legend()
            plt.tight_layout()
    ...
    with open('{}.astero.in'.format(profile), 'w+') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('profile',nargs='?',help='results will be stored in <profile>.astero.in. of the form profileN.data')
