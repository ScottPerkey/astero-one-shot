...
from .dnu_util import dnu_e, lorentzians, lorentzians_full, lorentzians_full_ell
...
def get_model(mod, amps, numax, **kwds):
    ...
    _amps = np.array([1.0, 0.4, 0.5]) # these are by eye from Huber+ 2010 fig. 10
    ...
    # from Huber+ 2010
    # these are the separations from the ell = 0 mode for : ell = 0, ell = 1, ell = 2, ell = 3.
    # the 01 difference is just taken by eye from Fig. 9.
    # note they are all fractional in terms of dnu, so that to get the actual separations, xs, there is a part that is multiplied by dnu and then a constant offset.
